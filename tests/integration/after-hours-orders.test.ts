...
describe('after-hours order queuing', () => {
  it('queues a buy without moving cash, then fills it at the next open', async () => {
    ...
    const queued = await queueBuyOrder(
      { orderId, virtualAccountId: account.id, instrumentId: instrument.id, amountPaise: 50_000n },
      database,
      prices,
    );
    ...
    await expectReconciled(account.id, INITIAL_BALANCE_PAISE - 50_000n); // 5 × ₹100
  });
    ...
    const queued = await queueBuyOrder(
      {
        ...
        amountPaise: INITIAL_BALANCE_PAISE + 10_000n, // more than the account holds
      },
      database,
      prices,
    );
  ...
  it('queues and fills a sell of owned shares', async () => {
    ...
    await queueBuyOrder(
      { orderId: buyId, virtualAccountId: account.id, instrumentId: instrument.id, amountPaise: 100_000n },
      database,
      prices,
    );
